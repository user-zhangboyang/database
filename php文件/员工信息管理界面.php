    ...
    <div class="d1">
        ...
        <div class="add">
            <form action="./staffselect.php" method="post" name="search" onsubmit = "return check()">
                ...
                <input class="in" type="submit" name="submit" value="搜索" style="height:30px;background-color:green;color:white;font-size:15px">
                <a href="./staffadd.php">添加</a>
                <a href="./管理员界面.php">返回</a>
            </form>
        </div>
        <!-- 表头 -->
        <table class="table">  
            ...
            class page{
                ...
                function show($url){       
                    ...
                    $html .= "<a class='page' href='$url?p={$backnum}'>上一页</a>";
                    ...
                    for($i=1;$i<=$this -> pagemax;$i++){
                        if($this -> current == $i){
                            ...
                            $html .= "<a class='page' href='$url?p={$i}'>{$i}</a> ";
                        }
                    }
                    ...
                    $html .= "<a class='page' href='$url?p={$nextnum}'>下一页</a>";
            ...
            $sql = "select * from staff order by staff_id asc limit $start,10";
            $result = mysqli_query($conn,$sql);
            $length = mysqli_num_rows($result);
            if($length>0){
                for($i=0;$i<$length;$i++){
                    ...
                    echo "<td>
                              <a href='javascript:del({$row['staff_id']})'><font color=red>删除</a>
                              <a href='staffrevise.php?id={$row['staff_id']}'><font color=green>修改</a>
                          </td>";
                    echo "</tr>";
                }
            }
...
<script type="text/javascript">
    function del(id){
        if(confirm("是否确定删除？")){
            window.location = "staffdelete.php?id="+id;
        }
    }
</script>
